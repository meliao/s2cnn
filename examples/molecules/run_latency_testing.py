...
def inference_batch_s2cnn(mlp, s2cnn, data, batch_idxs, device_id=0):
    """ evaluate a batch for the s2cnn """
    geometry = data["features"]["geometry"][batch_idxs, ...]
    atom_types = data["features"]["atom_types"][batch_idxs, ...]
    atom_types_one_hot = to_one_hot(atom_types, NUM_ATOM_TYPES)
...
def train_s2cnn(mlp, s2cnn, data, train_batches, test_batches, num_epochs,
    ...
    for epoch in range(num_epochs):
        ...
        for iteration, batch_idxs in enumerate(test_batches):
            ...
            loss = eval_batch_s2cnn(mlp, s2cnn, data, batch_idxs, criterion)
            test_losses.append(loss.item())
        total_inference_time = default_timer() - inference_time_start
        logging.info("Batch Inference completed in %f seconds = %f seconds / sample", total_inference_time, total_inference_time / test_batches.indices.shape[0])


        #######################################################################
        # TIME INFERENCE ON ONE SAMPLE

        s2cnn.eval()
        mlp.eval()
        inf_times = []
        for i in range(100):
            batch_idx = np.array([i])
            inf_time_start = default_timer()
            pred = inference_batch_s2cnn(mlp, s2cnn, data, batch_idx)
            inf_time = default_timer() - inf_time_start

            inf_times.append(inf_time)

        inf_times = np.array(inf_times[1:])
        logging.info("Inference Latency mean: %f, max: %f, min: %f", inf_times.mean(), inf_times.min(), inf_times.max())
...
def main():
    ...
    train_s2cnn(mlp, 
                ...
                IndexBatcher(test_idxs, args.batch_size_s2cnn,
                             cuda=args.device_id),
                args.num_epochs_s2cnn, 
                args.init_learning_rate_s2cnn,
